Reconstruct forensic timelines for every case directory under `/app/data/work/`. The normative contract is `/app/data/docs/CONTRACT.md`. A disclosed fit pack is at `/app/data/fit/alpha/` (ledgers plus `/app/data/fit/alpha/smoke_digests.json`). A multi-module helper package is shipped under `/app/engine/` (orchestration in `pipeline.py`, with a thin `starter.py` re-export). That package is an almost-correct, fit-smoke-calibrated scaffold: digests under `smoke_digests.json` are sha256 of the JSON text the pipeline emits for the fit ledgers, not graded normative expecteds for work cases. Induce the closed rules from the contract, use the fit ledgers and engine smoke as a non-normative check, then apply the full contract to each work case. Held cases may compose multiple rules on one long timeline and are not isomorphic copies of the fit pack.
