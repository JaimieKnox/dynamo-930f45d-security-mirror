Reconstruct forensic timelines for every case directory under `/app/data/work/`. The normative contract is `/app/data/docs/CONTRACT.md`. A disclosed fit pack is at `/app/data/fit/alpha/` (ledgers plus `/app/data/fit/alpha/smoke_digests.json`). A partial helper is shipped at `/app/engine/starter.py`. That helper is incomplete and is only a fit smoke scaffold: its digests under `smoke_digests.json` are sha256 of the JSON text it emits for the fit ledgers, not graded normative expecteds for work cases. Induce the closed rules from the contract, use the fit ledgers and starter smoke as a non-normative check, then apply the full contract to each work case. Held cases may compose multiple rules on one timeline and are not isomorphic copies of the fit pack.
